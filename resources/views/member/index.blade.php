...
@section('title', 'Members - '.$mod->name)
 @section('content')
...
<div class="w-full bg-white rounded-lg border text-left mb-5">
    ...
    <div class="grid grid-cols-12 px-4 text-sm py-3 border-b last:border-0 even:bg-gray-50">
        ...
        <div class="col-span-3">{{ $member->user->full_name }}</div>
        <div class="col-span-5">{{ $member->user->email }}</div>
        <div class="col-span-2">{{ $member->note }}</div>
        @if ($mod->id_prof == auth()->id())
        <div class="col-span-1">
            <div class="flex justify-end">
                <a class="text-xs font-bold text-blue-600 hover:text-blue-800 transition" href="{{ route('members.delete', ['id' => $member->module_id, 'member' => $member->user_id]) }}">Supprimer</a>
            </div>
        </div>
        @endif
    </div>
